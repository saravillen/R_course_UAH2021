...
# (tip: colour="blue")
# $$$ ANSWER:
ggplot(dat, aes(x=xval, y=yval)) + geom_point(colour="blue")
...
ggplot(dat, aes(x=xval, y=yval)) +
  geom_point()
# $$$ ANSWER: always in the line above, for ggplot to understand that
# there is still some information to read in the lines below
...
ggplot(heightweight, aes(x=ageYear, y=heightIn)) + 
  geom_point(aes(size=weightLb),alpha=0.2)
...
# $$$ ANSWER:
sa + stat_bin2d(bins=70)
sa + stat_bin2d(size=5)
...
# Does it change? why?
# $$$ ANSWER: yes. positions are defined randomly each time
...
# $$$ ANSWER: 
sc + geom_point(position=position_jitter(width=0.1, height=3))
...
# $$$ ANSWER:
# width>0 in the first one and height>0 in the second one
...
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  geom_point() +
  geom_rug(position=position_jitter(width=0, height=0.4))

# $$$ Exercise*: jitter both axis rugs
# $$$ ANSWER:
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  geom_point() +
  geom_rug(position=position_jitter(0.3))
...
# $$$ ANSWER: 
?geom_smooth
sr + geom_point() + stat_smooth(method=lm, level=0.99)

# $$$ Exercise: find how to hide confidence interval
# $$$ ANSWER: 
sr + geom_point() + stat_smooth(method=lm, se=F)
sr + geom_point() + stat_smooth(method=lm, level=F)
sr + geom_point() + geom_smooth(method=lm, se=F)

# $$$ Exercise*: colour the line in red, and CI in pink
# $$$ ANSWER: 
sr + geom_point() + stat_smooth(method=lm, colour="red", fill="pink")

# $$$ Exercise: group points by sex and adjust a line for each sex
# $$$ ANSWER: two options:
sr + 
  geom_point(aes(colour=sex)) + 
  stat_smooth(aes(colour=sex, fill=sex))
...
# $$$ ANSWER: 
# 1- Run the model
# 2- Predict the adjusted curve across values in x-axis (using predict())
# 3- Represent the line with geom_line()
# 4- Confidence intervals using lines or geom_ribbon() (see below)
...
# $$$ ANSWER:
ggplot(clim, aes(x=Year, y=Anomaly10y)) +
  geom_line(aes(y=Anomaly10y-Unc10y), colour="red", linetype="dotted", size=1) +
  geom_line(aes(y=Anomaly10y+Unc10y), colour="red", linetype="dotted", size=1) +
  geom_line()
...
# $$$ Exercise: draw a black line above the areas in the following graphs 
ggplot(uspopage, aes(x=Year, y=Thousands, fill=AgeGroup)) +
  geom_area(alpha=0.4)
ggplot(uspopage, aes(x=Year, y=Thousands, fill=AgeGroup)) +
  geom_area(position="identity", alpha=0.2)
...
# $$$ ANSWER:
ggplot(cabbage_exp, aes(x=Date, y=Weight, fill=Cultivar)) +
  geom_bar(stat="identity", width=0.6, position=position_dodge(0.2))
...
# $$$ ANSWER:
ggplot(pg_mean, aes(x=group, y=weight)) +
  geom_bar(stat="identity", fill="lightblue", colour="black", linetype="dashed")
...
csub$pos1 <- ifelse(csub$Anomaly10y >= 0, 1, 0)

csub$pos2[csub$Anomaly10y>=0] <- 1
csub$pos2[csub$Anomaly10y<0] <- 0

csub$pos3 <- cut(csub$Anomaly10y,
                 breaks=c(-Inf,0,Inf), 
                 labels=c("Negative","Positive"))


ggplot(csub, aes(x=Year, y=Anomaly10y, fill=pos3)) +
  geom_bar(stat="identity")

# Directly:
ggplot(csub, aes(x=Year, y=Anomaly10y, 
                 fill=ifelse(Anomaly10y >= 0, "Positivo", "Negativo"))) +
  geom_bar(stat="identity")
...
# $$$ Exercise: guess how to complete the following graph to make a frequency polygon
ggplot(faithful, aes(x=waiting))
# $$$ ANSWER:
ggplot(faithful, aes(x=waiting)) + 
  geom_freqpoly()
...
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_boxplot(outlier.alpha=0.01)   # ... or outliers in a lighter colour...
...
# $$$ ANSWER:
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="area")  # all violins have the same area (before trimming the tails) (default)  
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="count") # areas are scaled proportionally to the number of observations
ggplot(diamonds, aes(x=cut, y=price)) +
  geom_violin(colour="purple", fill="lightblue", scale="width") # all violins have the same maximum width
...
# $$$ ANSWER:
ggplot(heightweight, aes(x=sex, y=ageYear)) +
  geom_violin(colour="brown", fill="orange", adjust=0.3) # level of estimation adjustment to data
...
# $$$ ANSWER:
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  stat_density2d(aes(alpha=stat(density)), geom="raster", contour=FALSE)
...
# $$$ ANSWER:
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  stat_density2d(aes(colour=..level..), size=2)
...
ggplot(faithful, aes(x=eruptions, y=waiting)) + 
  stat_density2d(aes(fill=..density..), 
                 geom="raster", contour=FALSE, h=c(2,2)) 
...
############# #
#### 14.1. ## Spatial data in dataframe: geom_polygon(), geom_point(), geom_path(),geom_raster() ####
####################################################################################### #
...
ggplot() + 
  ...
  geom_point(data = world.cities, 
             aes(x = long, y = lat), colour="red", size=0.5) + 
  coord_quickmap()
...
ggplot() + 
  geom_polygon(data = map_data("world"), aes(x = long, y = lat, group = group)) + 
  geom_point(data = world.cities, 
             aes(x = long, y = lat, colour=country.etc), size=0.5) + 
  coord_quickmap()+
  theme(legend.position = "none")
...
ggplot() + 
  ...
  coord_quickmap()
# Start and end connected probably because data does not start at the beginning of each river.
# https://stackoverflow.com/questions/53432634/leaving-the-path-open-from-geom-path
...
# $$$ ANSWER:
ggplot() + 
  geom_raster(data=tmindf, mapping=aes(x=x, y=y, fill = tmin3)) + 
  coord_quickmap() 
...
ggplot() + 
  geom_polygon(data = map_data("world"), aes(x = long, y = lat, group = group)) + 
  coord_quickmap(xlim = c(-10, 4),  ylim = c(35, 44))
...
ggplot() +
  layer_spatial(data = longlake_depth_raster) +
  layer_spatial(data = longlake_waterdf, fill="NA") +
  layer_spatial(data = longlake_buildingsdf) +
  layer_spatial(data = longlake_roadsdf) 
...
## Colour chroma (default: c = 100) (max depends on hue and lightness)
################# #
...
ggplot(heightweight, aes(x=ageYear, y=heightIn, colour=weightLb, size=weightLb)) +
  ...
  labs(tag="B", 
       caption="Figure 1. Here you can insert a note or figure caption.", 
       colour = "Legend title")  + 
  scale_colour_continuous(name= "Legend title")+
...
# $$$ ANSWER:
p + annotate("segment", x=1970, xend=1945, y=.9, yend=.2, 
...
# Static plots
flower <- ggplot(iris, aes(x = Petal.Width, y = Petal.Length, 
                           colour=Species)) + 
  geom_point()
flower
...
# $$$ ANSWER:
flower_states_title <- flower_states +
  labs(title = "Flowers of the species: {closest_state}")
flower_states_title
...
# Tip: view_follow()
# $$$ ANSWER:
air_reveal_view <- air_reveal + view_follow()
air_reveal_view
...
# $$$ ANSWER:
gm_time_wake <- gm_time + 
  shadow_wake(wake_length = 0.15, # number between 0 and 1
              wrap=FALSE)         # last data affect first data?
gm_time_wake


# $$$ Exercise*: Now use shadow_mark() to leave a mark at point positions
# $$$ ANSWER:
gm_time_mark <- gm_time +
  shadow_mark()         
gm_time_mark
